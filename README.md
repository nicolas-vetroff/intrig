...
Sur **desktop**, `/books` épingle la barre de titre et l'aside de filtres en `position: fixed` : seule la liste des livres scrolle avec la page. Layout à deux colonnes (**filtres à gauche**, **livres à droite**). Sur **mobile**, pas de verrou : la page scrolle d'un bloc — titre et filtres défilent avec la liste. Chaque carte affiche le titre, le genre, l'auteur, un synopsis **tronqué à ~130 caractères** avec `…`, et une **couverture de taille fixe au ratio 2:3** (même footprint partout). Filtres côté client (`app/(marketing)/books/_components/catalog-browser.tsx`) :
