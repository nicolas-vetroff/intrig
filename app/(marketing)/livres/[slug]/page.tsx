...
export default async function BookDetailPage({
  ...
  const lireUrl = `/livres/${book.slug}/lire`
  const ctaHref = user ? lireUrl : `/connexion?next=${encodeURIComponent(lireUrl)}`
  ...
  return (
    <article className="mx-auto flex max-w-2xl flex-col gap-10 px-6 py-16 sm:px-10 sm:py-20">
      ...
      <div className="flex flex-col gap-3">
        <Link
          href={ctaHref}
          className="bg-foreground text-background self-start rounded-md px-6 py-3 text-sm font-medium transition-opacity hover:opacity-90"
        >
