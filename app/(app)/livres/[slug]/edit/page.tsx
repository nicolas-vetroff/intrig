...
import { updateBook } from '@/app/(app)/livres/_actions/admin'
...
export default async function EditBookPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  await requireAdmin(`/livres/${slug}/edit`)
  ...
  return (
    <section className="mx-auto flex max-w-3xl flex-col gap-8 px-6 py-16 sm:px-10 sm:py-20">
      ...
      <BookForm action={updateBook} initial={book} submitLabel="Enregistrer" />
    </section>
  )
}
