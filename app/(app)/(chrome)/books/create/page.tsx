...
import { createBook } from '@/app/(app)/books/_actions/admin'
...
export default async function CreateBookPage() {
  ...
  // The username guard in proxy.ts ensures a username exists by the
  // time an admin reaches this page. Defensive fallback just in case.
  const profile = await getCurrentProfile()
  if (!profile?.username) redirect('/account/choose-username?next=/books/create')
  ...
  return (
    <section className="mx-auto flex max-w-3xl flex-col gap-8 px-6 py-16 sm:px-10 sm:py-20">
      ...
      <BookForm action={createBook} authorDisplay={profile.username} submitLabel="Créer le livre" />
    </section>
  )
}
